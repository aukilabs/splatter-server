...
impl compute_runner_api::Runner for HelloRunner {
    ...
    async fn run(&self, ctx: compute_runner_api::TaskCtx<'_>) -> Result<()> {
        ...
        let task_result: Result<()> = async {
            ...
            // Upload preview images if they exist (best-effort, non-fatal)
            let preview_top = job_root.join("refined").join("splatter").join("preview_top.png");
            let preview_angle = job_root.join("refined").join("splatter").join("preview_angle.png");
            let mut preview_uploaded = Vec::new();

            if preview_top.exists() {
                let preview_key = if let Some(suffix) = refined_suffix.as_deref().filter(|s| !s.is_empty()) {
                    if suffix.starts_with('_') {
                ...
                match ctx.output
                    .put_domain_artifact(compute_runner_api::runner::DomainArtifactRequest {
                        rel_path: preview_key.as_str(),
                        name: preview_key.as_str(),
                        data_type: "splat_preview_image",
                        existing_id: None,
                ...
                {
                    Ok(_) => {
                        preview_uploaded.push(preview_key.clone());
                        info!(preview_key = %preview_key, "uploaded top-down preview image");
                    }
                    Err(err) => {
                        warn!(preview_key = %preview_key, %err, "failed to upload top-down preview image (non-fatal)");
                    }
                }
            }
            ...
            if preview_angle.exists() {
                    ...
                    .await
                {
                    Ok(_) => {
                        preview_uploaded.push(preview_key.clone());
                        info!(preview_key = %preview_key, "uploaded angled preview image");
                    }
                    Err(err) => {
                        warn!(preview_key = %preview_key, %err, "failed to upload angled preview image (non-fatal)");
                    }
                }
            }
            ...
            ctx.ctrl
                .progress(json!({
                    ...
                    "stage": "complete",
                    "status": "succeeded",
                    "preview_images": preview_uploaded,
                }))
                .await?;
